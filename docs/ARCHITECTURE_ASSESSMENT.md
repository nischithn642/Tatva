...
`relax.build(target="c")` → TVM's C source → optional textual softmax swap → TATVA emits `weights.h`, `model_run.c`, `model_info.h`, `operators.c`, `start.S`, `link.ld`, `main.c` from module-level string constants → `riscv-none-elf-gcc -O2 -march=<...> -mabi=<...> -mcmodel=medany -ffreestanding -nostdlib` → `model.elf` → `qemu-system-riscv64 -M virt -kernel -nographic -icount shift=0` (`runner.py:1423-1441`, 30 s subprocess timeout).
